...
import { deleteVehicleById } from 'apiSdk/vehicles';
...
function CompanyViewPage() {
  ...
  const vehicleHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('vehicle', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/vehicles/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.vehicle?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/vehicles/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.model}</Td>
                            <Td>{record.type}</Td>
